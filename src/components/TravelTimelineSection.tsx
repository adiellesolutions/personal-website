...
const TimelineEvent = ({ title, location, date, days, description, rating, photos, img }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-4 flex flex-col gap-2 w-full md:w-[45%] transition-all duration-300">
    <div className="flex items-center gap-3">
      <img src={img} alt={title} className="w-12 h-12 rounded-full object-cover border border-gray-200 dark:border-gray-700" />
      <div>
...
const TimelineYear = ({ year, summary, details, children, isOpen, onToggle }) => (
  <div className="flex items-start gap-6">
    ...
    <div className="flex flex-col items-center">
      <div className="bg-gradient-to-r from-pink-400 to-blue-400 text-white font-bold px-4 py-2 rounded-full shadow-md">
        {year}
      </div>
      <div className="w-1 h-full bg-pink-200 dark:bg-pink-600"></div>
    </div>

    {/* Year Content */}
    <div className="flex-1 space-y-3">
      <div
        className="bg-white dark:bg-gray-800 shadow rounded-2xl p-4 flex justify-between items-center cursor-pointer transition-all duration-300 hover:shadow-lg"
        onClick={onToggle}
      >
        <div>
          <h2 className="font-semibold text-gray-800 dark:text-gray-100">{summary}</h2>
